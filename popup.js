document.addEventListener("DOMContentLoaded", function() {
    ...
    // browser.browserAction.onClicked.addListener(jiraPopup);
    ...
    // browser.storage.sync.get(['recent_count'], function(items) {
    //     recent_count = items.recent_count;
    // // alert(recent_count)
    //     browser.history.search({text: 'atlassian.net', startTime: kFourWeekAgo, maxResults: (Number(recent_count) || 10)}, function(items){
    ...
    var gettingRecentCount = browser.storage.sync.get(['recent_count']);
    gettingRecentCount.then(function(items) {
        recent_count = items.recent_count;
        browser.history.search({
            text: 'atlassian.net',
            startTime: kFourWeekAgo,
            maxResults: (Number(recent_count) || 10)
        }, function(items) {
